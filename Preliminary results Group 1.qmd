...
##PCA
#ERROR TO FIX: 
Quitting from lines 287-358 [unnamed-chunk-3] (Preliminary-results-Group-1.rmarkdown)
Error in `colMeans()`:
! 'x' debe ser numérico
```{r}
library(dplyr)
...
pcadata <- newdata[c(4:7,11,12,16,19,24,25)]
#ERROR CHECK Variable instead of root water content
...
ggcorrplot(corr_matrix)#displays a correlation matrix NOT VISUAL
#...working
...
pca1 <- fviz_pca_var(data.pca, col.var = "black")
pca1
fviz_cos2(data.pca, choice = "var", axes = 1:2)
pca2 <- fviz_pca_var(data.pca, col.var = "cos2",
            gradient.cols = c("red", "purple", "blue"), #chosen gradient
            repel = TRUE,
            display = c("Use", "Species"), #display other variables
            type = c("points", "text")) #how to display them
pca2
#last part does not display Use and species

library(vegan)

plot.pca <- rda(pcadata[5:10])

plottedpca <- biplot(plot.pca,
     display = c("sites", "species"),
     type = c("text", "points"))
#need to tidy the veganplot
##NOT WORKING
#ordihull(vegan.pca,
