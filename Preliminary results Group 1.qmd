...
pca2 <- fviz_pca_var(data.pca, col.var = "cos2",
            gradient.cols = c("darkred", "purple", "blue"),
            repel = TRUE)
pca2

#pca dots IN PROGRESS NOT WORKING

#ggplot(pcadata, aes(x=PC1, y=PC2, color=Species)) + geom_point())

#library(ggplot2)
#pcadata <- as.numeric(as.factor(pcadata))
#pcadata <- prcomp(pcadata[c(1:10)], scale.=T)

#df$PC1 <- pcadata$x[,1]
#df$PC2 <- pcadata$x[,2]
#df$species <- pcadata$Species
